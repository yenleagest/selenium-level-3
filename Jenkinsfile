pipeline {
    ...
    parameters {
        ...
        choice(name: 'ENVIRONMENT',
                choices: ['https://www.agoda.com', 'https://books-pwakit.appspot.com', 'https://store.leapfrog.com', 'https://siassistance.com/en', 'https://www.vietjetair.com/en', 'https://www.vietjetair.com/vi'],
                description: 'Specify an environment to run tests.')
        choice(name: 'SUITE_NAME',
                choices: ['AgodaRegression', 'BooksRegression', 'LeapFrogRegression', 'SIARegression', 'VJRegression'],
        ...
        choice(name: 'DATA_DRIVEN_OUTPUT',
                choices: ['true', 'false'],
                description: 'Record data-driven test results in its input file.')
    }
    stages {
        stage('Initialize') {
            steps {
        ...
        stage('Run Tests') {
            steps {
                script {
                    ...
                    if (params.ENVIRONMENT.contains("leapfrog")) {
                        params.ENVIRONMENT = "https://store.leapfrog.com/en-us/apps/c?p=1&platforms=197&product_list_dir=asc&product_list_order=name"
                    }
